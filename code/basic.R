...
trace_plot <- function(x, y){
  
  plot(x$`1`[[y]], type = 'l', col = 'darkgreen', ylab = y, main = paste('Trace plot of', y))
  lines(x$`1`[[y]], col = 'tomato')
  lines(x$`3`[[y]], col = 'darkviolet')
  lines(x$`4`[[y]], col = 'darkcyan')
  
}

running_means <- function(x, y){
  
  plot(cumsum(x$`1`[[y]])/seq(along=x$`1`[[y]]), main = paste("Running mean of ", y), lwd = 2, type="l", col = 'darkgreen', ylab = y)
  lines(cumsum(x$`2`[[y]])/seq(along=x$`2`[[y]]), col = 'tomato')
  lines(cumsum(x$`3`[[y]])/seq(along=x$`3`[[y]]), col = 'darkviolet')
  lines(cumsum(x$`4`[[y]])/seq(along=x$`4`[[y]]), col = 'darkcyan')
...
x = as_draws(fit_basic)

pars = c("sd_constructor__Intercept", "sd_constructor:year__Intercept",
         "sd_rider__Intercept", "sd_rider:year__Intercept", "phi")

par(mfrow = c(5, 1))
for(param in pars) trace_plot(x, param)

rstudioapi::savePlotAsImage("./img_basic/chains.png",width=800,height=1000)
...
rstudioapi::savePlotAsImage("./img_basic/running_means.png",width=800,height=1000)
...
ggsave("./img_basic/rhats.png", width = 14, height = 8)
...
pp_tab <- posterior_predict(fit_basic, pred_tab)
...
true_tab_long <-
  ...
  filter(year == 2019) %>%
  group_by(rider, constructor, year) %>% filter(n() > 5) %>%
  select(rider, constructor, year, prop_trans) %>%
  ungroup() %>%
...
ggsave("./img_basic/prop_2019.png", width = 15, height = 12, bg = "white")
...
pp_ranks <-
  apply(pp_tab, 1, function(x) rank(-x))[,1] %>%
  apply(1, reweigh) %>%
  t() %>%
...
true_rank_long <-
  ...
  filter(year == 2019) %>%
  group_by(rider, constructor, year) %>% filter(n() > 5) %>%
  select(rider, constructor, year, position) %>%
  mutate(origin = "observed")
...
ggsave("./img_basic/pp_check_rank_2019.png", width = 20, height = 14, bg = "white")
...
rider_mean <- as_draws_df(fit_basic, variable = "r_rider\\[.+Intercept]", regex = TRUE) %>% select(-.chain, -.iteration)
rider_form <- as_draws_df(fit_basic, variable = "r_rider:year\\[.+Intercept]", regex = TRUE) %>% select(-.chain,-.iteration)
...
ggsave("./img_basic/plt_skill_trajectories.png", plot = plt_skill_trajectory, width = 12, height = 9, bg = "white")
...
ggsave("./img_basic/plt_skill_2021.png", plot = plt_rider_skill_2021, width = 9, height = 9, bg = "white")


# ------- Inference about constructor advantage ------- #

constructors_focus <- c("Repsol Honda Team", "Ducati", "Team SUZUKI ECSTAR", "Yamaha Factory", "Aprilia Racing Team", "LCR Honda")

constructor_mean <- as_draws_df(fit_basic, variable = "r_constructor\\[.+Intercept]", regex = TRUE) %>% select(-.chain, -.iteration)
constructor_form <- as_draws_df(fit_basic, variable = "r_constructor:year\\[.+Intercept]", regex = TRUE) %>% select(-.chain,-.iteration)
...
plt_advantage_trajectory <-
  ...
  mutate(constructor = fct_relevel(constructor, "Repsol Honda Team", "Ducati", "Team SUZUKI ECSTAR", "Yamaha Factory", "Aprilia Racing Team", "LCR Honda")) %>%
  ggplot(aes(x = year, y = est, ymin = lower, ymax = upper)) +
...
ggsave("./img_basic/plt_advantage_trajectory.png", plot = plt_advantage_trajectory, width = 12, height = 9, bg = "white")
...
ggsave("./img_basic/plt_advantage_avg.png", plot = plt_advantage_avg, width = 9, height = 6, bg = "white")



# ------- rider versus constructor contributions ------- #

sfit <- summary(fit_basic, prob = 0.89)

ranef_summary <- rbind(
  "constructor" = sfit$random$constructor,
  "constructor form" = sfit$random$`constructor:year`,
  "rider" = sfit$random$rider,
  "rider form" = sfit$random$`rider:year`
)[1:4, 5:7]

xtable::xtable(round(ranef_summary, 2))

# how much of variance is due to bike?
vars = rbind('bike_var' = colSums(ranef_summary[1:2,]^2)/colSums(ranef_summary^2),
# and how much due to the rider?
'rider var' = colSums(ranef_summary[3:4,]^2)/colSums(ranef_summary^2))
...
pp_2021 <- posterior_predict(fit_basic, grid_2021)
...
ggsave("./img_basic/plt_performance_2021.png", width = 9, height = 6, bg = "white")


# Counter factual example --------------------------------------------------

# in a race in 2018, how likely is it that Dovizioso in a Honda beats Marquez in a Ducati?

dovizioso_yamaha <- posterior_predict(fit_basic, tibble(
  year = 2018,
  constructor = "Yamaha Factory",
  rider = "Andrea Dovizioso"
))

marquez_EG <- posterior_predict(fit_basic, tibble(
  year = 2018,
...
ggsave("./img_basic/plt_counterfactual.png", width = 9, height = 5, bg = "white")
